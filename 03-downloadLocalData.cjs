...
const s3 = new AWS.S3();
const bucketName = "local-career-files";
...
async function downloadCSVForStates(OccupationCode, states) {
  ...
  const downloadTasks = states.map(async (state, index) => {
    ...
    const fileUrl = await page.evaluate((selector) => {
      const link = document.querySelector(selector);
      return link.href;
    }, selector);
    await uploadToS3(fileUrl, `${OccupationCode}_${state}.csv`);
  });
  await Promise.all(downloadTasks);
  await browser.close();
}

async function uploadToS3(fileUrl, fileName) {
